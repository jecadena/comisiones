  ...
  cargarFechasFacturacion() {
    this.comisionesService.getFechasFacturacion().subscribe(
      data => {
        ...
        // Para cada fecha, obtener las comisiones correspondientes
...
procesarConfirmacionComisiones() {
  ...
  const updateCommissionsPromise = existingCommissions.length > 0 ? 
      new Promise<void>((resolve, reject) => {
          ...
          this.http.post('http://localhost:3000/api/update-comisiones', formatCommissions(existingCommissions))
              .subscribe({
                  next: () => {
  ...
  const insertCommissionsPromise = newCommissions.length > 0 ?
      new Promise<void>((resolve, reject) => {
          ...
          this.http.post('http://localhost:3000/api/insert-comisiones', formatCommissions(newCommissions))
              .subscribe({
                  next: () => {
                      updateProgressBar(100);
                      setTimeout(() => {
...
compararDatos(data: any[][]) {
  ...
  const checkUrl = `http://localhost:3000/api/compare-comisiones?codes=${encodedCodes}&fechaFacturacion=${encodedFechaFacturacion}&lastName=${encodedLastName}&firstName=${encodedFirstName}`;
...
procesarBusqueda(encodedCodes: string, fechaFacturacion: string, filteredData: any[], firstName: string, lastName: string) {
  ...
  const url = `http://localhost:3000/api/compare-comisiones?codes=${encodedCodes}&fechaFacturacion=${fechaFacturacion}&firstName=${firstName}&lastName=${lastName}`;
  ...
  this.http.get<{ found: boolean, data: any[], conflict?: boolean, message?: string }>(url).subscribe(
      (response) => {
          ...
          filteredData.forEach((item: any) => {
              ...
              this.http.get<{ de_vendedor: string }>(`http://localhost:3000/api/get-vendedor?firstName=${item.firstName}&lastName=${item.lastName}`)
                  .subscribe(
                      (vendedorResponse) => {
